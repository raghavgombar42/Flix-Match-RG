      ...
      <div className="flex flex-col gap-3 lg:grid lg:grid-cols-2 lg:gap-4 xl:grid-cols-3">
        {topFive.map((title, i) => (
          <button
            ...
            className="btn-focus group flex items-center gap-4 rounded-2xl border border-ink-600 bg-ink-800/60 p-3 text-left transition-all hover:border-ember-400/50 hover:bg-ink-700/60 lg:flex-col lg:items-stretch lg:gap-3"
          >
            ...
            <div className="hidden lg:flex lg:flex-col lg:gap-3">
              <div className="relative aspect-[2/3] w-full overflow-hidden rounded-xl">
                <PosterImage seed={title.posterSeed} url={title.posterUrl} title={title.name} className="h-full w-full" />
                <span className="absolute left-2 top-2 flex h-6 w-6 items-center justify-center rounded-full bg-ink-950/80 font-display text-xs font-bold text-ember-400">
                  {i + 1}
                </span>
              </div>
              ...
              <span className="rounded-full border border-ink-500 px-3 py-1.5 text-center text-xs font-semibold text-ember-400 transition-colors group-hover:border-ember-400/60">
                Pick this one
              </span>
            </div>
          </button>
